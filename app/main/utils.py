...
def combined_analyze(
    ...
    **kwargs
    ...
    Parameters:
            ...
            enforce_actions (boolean): Set this to False, it will not check if any action is passed,
            otherwise, it will check if any action is passed and raise exception if not.

            silent (boolean): disable (some) log messages

            kwargs: pass arguments to extract_face function (like enforce_detection, detection_backend, and align).
    ...
    Raises:
            ValueError: "actions" should be a non empty list or tuple.
    """
    # ---------------------------------
    if (actions == tuple() or actions == []) and enforce_actions == True:
        raise ValueError(
            "'actions' is a required field for analyze function. Please pass a non empty list or tuple.")
...
def pretty_result(result: dict | str, settings_json: dict):
    ...
    return render_template('infocards.htm', result={
        'settings': settings_json, 'faces': result})
